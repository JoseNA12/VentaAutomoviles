...
AS 
	...
	IF EXISTS(SELECT branchOffice_id FROM BranchOffice WHERE branchOffice_id = @office_id)
		SELECT cs.[car_stock_id], cs.[stock_id], cs.[quantity], c.car_id, c.carBrand_id, cb.name, c.carType_id, ct.name, c.model, c.engine, c.year, c.seats, c2.doors, c2.fuelType_id, cf.name, c2.acceleration, c2.maximum_speed, c2.price, c2.photo, c2.production_date
		FROM [Car-Stock] cs 
